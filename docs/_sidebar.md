<!-- docs/_sidebar.md -->

- **Getting Started**
  - [Overview](/)
  - [Quick Start](/#quick-start)
  - [Installation](/#installation)
  - [Docker Usage](/#docker)

- **Reference**
  - [API Reference](API-REFERENCE.md)
  - [Architecture](ARCHITECTURE.md)
  - [Design Document](DESIGN.md)

- **Usage**
  - [Go Client Examples](/#use-with-gcp-sdk)
  - [Configuration](/#configuration)
  - [Testing Strategies](/#testing-strategies)

- **Implementation**
  - [Supported Operations](/#supported-operations)
  - [Error Codes](API-REFERENCE.md#error-codes)
  - [Resource Naming](API-REFERENCE.md#resource-naming-convention)
  - [Pagination](API-REFERENCE.md#pagination)

- **Project Info**
  - [Use Cases](/#use-cases)
  - [Differences from Real GCP](/#differences-from-real-gcp)
  - [Testing Guide](API-REFERENCE.md#testing-strategies)
  - [Contributing](API-REFERENCE.md#contributing)

- **Links**
  - [![GitHub](https://icongr.am/simple/github.svg?size=16&colored=false) GitHub Repo](https://github.com/blackwell-systems/gcp-secret-manager-emulator)
  - [![Issues](https://icongr.am/octicons/issue-opened.svg?size=16) Issues](https://github.com/blackwell-systems/gcp-secret-manager-emulator/issues)
  - [![GoDoc](https://icongr.am/simple/go.svg?size=16&colored=false) GoDoc](https://pkg.go.dev/github.com/blackwell-systems/gcp-secret-manager-emulator)
